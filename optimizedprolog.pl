%8,737 inferences, 0.000 CPU in 0.077 seconds
%% 678,311,116 inferences, 149.953 CPU in 208.003 seconds
width(25).
row(1,[7,3,1,1,7]).
row(2,[1,1,2,3,1,1]).
...
try(RowBoard) :- width(X),
		...
		sort_branching_cols(Numl,[],Branchorder),
		generate_pos_sol(Branchorder,[],RowBoard).

sort_branching_cols([X|Rest],Intermediate,Sorted) :- total_pos_col([X],L,1),
Tuples = [(L,X)|Intermediate],
sort_branching_cols(Rest,Tuples,Sorted).
sort_branching_cols([],Tuples,Sorted) :- sort( Tuples, Isorted),
findall(Y,member((_,Y),Isorted),Sorted).
...
generate_pos_sol([X|Rest],Sol,Out) :- pos_col(X,T),
							  ...
							  member(Candidate,T),
						append(Sol,[col(X,Candidate)],NewSol),
						...
						close(Stream),
						width(Width),
						numlist(1,Width,Numl),
						still_valid(NewSol,Numl),
						
						generate_pos_sol(Rest,NewSol,Out).
						
still_valid(Sol,[]).
still_valid(Sol,[Row|Rest]) :- solved_row(Row,Rows),
								valid_row_exists(Sol,Row,Rows),
								still_valid(Sol,Rest).
								
valid_row_exists(Sol,X,[]) :- false.
valid_row_exists(Sol,X,[Row|Rest]) :- valid_row_with_sol(Sol,X,Row,1).
valid_row_exists(Sol,X,[Row|Rest]) :- \+valid_row_with_sol(Sol,X,Row,1),
valid_row_exists(Sol,X,Rest).

valid_row_with_sol(Sol,X,[],At).
valid_row_with_sol(Sol,X,[Bit|Rest],At) :- \+ member(col(At,_),Sol),
										   Newat is At + 1,
										   valid_row_with_sol(Sol,X,Rest,Newat).
valid_row_with_sol(Sol,X,[Bit|Rest],At) :- member(col(At,Colm),Sol),
Index is X-1,
										   nth0(Index,Colm,Actual),
										   Actual = Bit,
										   Newat is At + 1,
										   valid_row_with_sol(Sol,X,Rest,Newat).
total_pos_col([],L,P) :- L = P.
total_pos_col([X|Rest],L,Product) :- 
		pos_col(X,Colm),length(Colm,Len), NewProduct is Len* Product, total_pos_col(Rest,L,NewProduct).
total_pos_row([],L,P) :- L = P.
total_pos_row([X|Rest],L,Product) :- 
